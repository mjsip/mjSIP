...
public class RtpPacket {
	...
	/** Creates a new RTP packet.
	  * @param data the RTP packet */ 
	public RtpPacket(byte[] data) {
		this(data,0,data.length);
	}

	/** Creates a new RTP packet.
	  * @param udp_packet UDP packet containing the RTP packet */ 
	public RtpPacket(UdpPacket udp_packet) {
		this(udp_packet.getData(),udp_packet.getOffset(),udp_packet.getLength());
	}
	...
	/** Initializes the RTP packet buffer. 
	...
	public int getPaddingLength() {
		if (length<HDR_LEN) return 0; // broken packet
		// else
		boolean p=BinUtils.getBit(buffer[offset],5);
	...
	public void setPaddingLength(int padding_len) {
		...
		if (padding_len>0)  {
			...
			BinUtils.setBit(true,buffer[offset],5);
		}
		else BinUtils.setBit(false,buffer[offset],5);
	}
	...
	public boolean hasExtension() {
		if (length<HDR_LEN) return false; // broken packet
		// else
		return BinUtils.getBit(buffer[offset],4);
	}
	...
	public void setExtension(boolean x) {
		if (length<HDR_LEN) return; // broken packet
		// else
		BinUtils.setBit(x,buffer[offset],4);
	}
	...
	public boolean hasMarker() {
		if (length<HDR_LEN) return false; // broken packet
		// else
		return BinUtils.getBit(buffer[offset+1],7);
	}
	...
	public void setMarker(boolean m) {
		if (length<HDR_LEN) return; // broken packet
		// else
		BinUtils.setBit(m,buffer[offset+1],7);
	}
	...
	public int getSequenceNumber() {
		if (length<HDR_LEN) return 0; // broken packet
		// else
		return BinUtils.getInt(buffer,offset+2,offset+4);
	}
	...
	public void setSequenceNumber(int sn) {
		if (length<HDR_LEN) return; // broken packet
		// else
		BinUtils.setInt(sn,buffer,offset+2,offset+4);
	}
	...
	public long getTimestamp() {
		if (length<HDR_LEN) return 0; // broken packet
		// else
		return BinUtils.getLong(buffer,offset+4,offset+8);
	}
	...
	public void setTimestamp(long timestamp) {
		if (length<HDR_LEN) return; // broken packet
		// else
		BinUtils.setLong(timestamp,buffer,offset+4,offset+8);
	}
	...
	public long getSsrc() {
		if (length<HDR_LEN) return 0; // broken packet
		// else
		return BinUtils.getLong(buffer,offset+8,offset+12);
	}
	...
	public void setSsrc(long ssrc) {
		if (length<HDR_LEN) return; // broken packet
		// else
		BinUtils.setLong(ssrc,buffer,offset+8,offset+12);
	}
	...
	public long[] getCsrcList() {
		...
		for (int i=0; i<cc; i++) csrc[i]=BinUtils.getLong(buffer,offset+HDR_LEN+4*i,offset+HDR_LEN+4*i+2); 
		return csrc;
	}
	...
	public void setCsrcList(long[] csrc) {
		...
		if (length>=(HDR_LEN+4*cc)) {
			setCsrcCount(cc);
			for (int i=0; i<cc; i++) BinUtils.setLong(csrc[i],buffer,offset+HDR_LEN+4*i,offset+HDR_LEN+4*i+2);   
	...
	/** Gets a UDP packet containing this RTP packet.
	  * @param dest_soaddr destination socket address */
	public UdpPacket toUdpPacket(SocketAddress dest_soaddr) {
		UdpPacket udp_packet=new UdpPacket(buffer,offset,length);
		udp_packet.setIpAddress(dest_soaddr.getAddress());
		udp_packet.setPort(dest_soaddr.getPort());
		return udp_packet;
	}
