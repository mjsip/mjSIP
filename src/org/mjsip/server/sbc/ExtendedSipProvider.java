...
public class ExtendedSipProvider extends org.mjsip.sip.provider.SipProvider {
	...
	private void init(long refresh_time, long keepalive_time) {
		if (keepalive_time>0) address_resolver=new AddressResolverKeepAlive(refresh_time,this,keepalive_time);
		else address_resolver=new AddressResolver(refresh_time);
	}


	/** Creates a new ExtendedSipProvider. 
	...
	@Override
	protected synchronized void processReceivedMessage(SipTransport transport, SipMessage msg) {
		...
		try {
			...
			log(LoggerLevel.WARNING,"Error managing addressing material");
			log(LoggerLevel.DEBUG,e);
		}

		super.processReceivedMessage(transport,msg);
	}
	...
	public ConnectionId sendMessage(SipMessage msg, String proto, String dest_addr, int dest_port, int ttl) {
		...
		log(LoggerLevel.INFO,"Sending message ("+msg.getLength()+" bytes): "+foot_print);
		...
		//printLog("DEBUG: refer_soaddr="+refer_soaddr.toString(),LoggerWriter.LEVEL_HIGH);
		//for (Enumeration e=address_resolver.getAllSocketAddresses(); e.hasMoreElements(); )
		//{  printLog("DEBUG: resolv_soaddr="+((String)e.nextElement()),LoggerWriter.LEVEL_HIGH);
		//}
		if (address_resolver.contains(refer_soaddr)) {
			dest_soaddr=address_resolver.getSocketAddress(refer_soaddr);
			log(LoggerLevel.INFO,"CHANGING DESTINATION "+refer_soaddr+" >> "+dest_soaddr);
			...
			log(LoggerLevel.INFO,"destination unchanged: "+dest_soaddr);
		}
	...
	private final void log(LoggerLevel level, String str) {
		...
		if (logger!=null) {
			...
			logger.log(level,"SipProviderX-"+provider_id+": "+str);  
		}    
	}
