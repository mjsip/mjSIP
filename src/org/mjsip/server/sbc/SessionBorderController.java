...
public class SessionBorderController extends Proxy {
	...
	/** Creates a new SessionBorderController.
	...
	private void init(ExtendedSipProvider sip_provider, ServerProfile server_profile, SessionBorderControllerProfile sbc_profile) {
		...
		log(LoggerLevel.INFO,"Available media ports: ["+sbc_profile.media_ports.elementAt(0)+":"+sbc_profile.media_ports.elementAt(sbc_profile.media_ports.size()-1)+"] ("+sbc_profile.media_ports.size()+")");
	}
	...
	/*public void processRequestToRemoteUA(SipMessage msg) {
		printLog("inside processRequestToRemoteUA(msg)",LoggerWriter.LEVEL_MEDIUM);
		msg=SipMangler.unmangleRequestLine(msg);
				...
				if (time>0) {
					...
					if (keepalive_daemons.containsKey(key)) {
						...
						if (!keepalive.isRunning()) {
							...
							log(LoggerLevel.DEBUG,"KeepAlive: restart: "+soaddr+" ("+time+"secs)");
						}
						else log(LoggerLevel.DEBUG,"KeepAlive: update: "+soaddr+" ("+time+"secs)");
					}
					else {
						keepalive=new SipKeepAlive(sip_provider,soaddr,sbc_profile.keepalive_time);
						keepalive_daemons.put(key,keepalive);
						log(LoggerLevel.DEBUG,"KeepAlive: start: "+soaddr+" ("+time+"secs)");
					}
					...
					if (keepalive_daemons.containsKey(key)) {
						...
						keepalive_daemons.remove(key);
						keepalive.halt();
						log(LoggerLevel.DEBUG,"KeepAlive: halt: "+soaddr);
					}
				}
			}
		}
	...
	private SipMessage mangleBody(SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside mangleBody()");
	...
	/*private SipMessage unmangleRequestUri(SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside mangleRequestUri(msg)");
	...
	/*private SipMessage mangleContact(SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside mangleContact()");
	...
	public static void main(String[] args) {
		...
		String[] ports=flags.getStringTuple("-m",2,null,"<fist_port> <last_port>","interval of media ports");
