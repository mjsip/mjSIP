...
public abstract class ServerEngine {
	...
	protected static final String[] LOCATION_SERVICE_CLASSES={ "org.mjsip.server.LocationServiceImpl", "org.mjsip.ldap.LdapLocationServiceImpl" };
	...
	protected static final String[] AUTHENTICATION_SERVICE_CLASSES={ "org.mjsip.server.AuthenticationServiceImpl", "org.mjsip.ldap.LdapAuthenticationServiceImpl" };
	...
	protected static final String[] AUTHENTICATION_SERVER_CLASSES={ "org.mjsip.server.AuthenticationServerImpl" };


	// *************************** abstract methods ***************************

	/** When a new request message is received for a remote UA */
	protected abstract void processRequestToRemoteUA(SipMessage req);

	/** When a new request message is received for a locally registered user */
	protected abstract void processRequestToLocalUser(SipMessage req);

	/** When a new request request is received for the local server */
	protected abstract void processRequestToLocalServer(SipMessage req);
	
	/** When a new response message is received */
	protected abstract void processResponse(SipMessage resp);
	...
	/** Creates a void ServerEngine */
	protected ServerEngine() {}


	/** Creates a new ServerEngine on SipProvider <i>provider</i>,
	...
	public ServerEngine(SipProvider provider, ServerProfile profile) {
		...
		sip_provider.addSelectiveListener(MethodId.ANY,this_sip_provider_listener);
			 
		// LOCAL DOMAINS
		log(LoggerLevel.INFO,"Domains="+getLocalDomains());
		...
		try {
			...
			Class[] parameter_types={ String.class };
			...
			log(LoggerLevel.INFO,"Error trying to use location service '"+location_service_class+"': use default class.");
		}
		...
		if (profile.clean_location_db)  {
			...
			log(LoggerLevel.DEBUG,"LocationService \""+profile.location_db+"\": cleaned\r\n");
		}
		else {
		...
		log(LoggerLevel.DEBUG,"LocationService ("+profile.authentication_service+"): size="+location_service.size()+"\r\n"+location_service.toString());
		log(LoggerLevel.DEBUG,"LocationService ("+profile.authentication_service+"): size="+location_service.size()+"\r\n"+location_service.toString());
		...
		if (server_profile.do_authentication || server_profile.do_proxy_authentication) {
			...
			try {
				...
				log(LoggerLevel.INFO,e);
				log(LoggerLevel.INFO,"Error trying to use authentication service '"+authentication_service_class+"': use default class.");
			}
			...
			log(LoggerLevel.DEBUG,"AuthenticationService ("+profile.authentication_service+"): size="+authentication_service.size()+"\r\n"+authentication_service.toString());
			...
			try {
				...
				Class[] parameter_types={ String.class, Class.forName("org.mjsip.server.AuthenticationService") };
				...
				log(LoggerLevel.INFO,e);
				log(LoggerLevel.WARNING,"Error trying to use authentication server '"+authentication_server_class+"': use default class.");
			}
			// use default authentication service
			if (as==null) as=new AuthenticationServerImpl(realm,authentication_service);
			log(LoggerLevel.DEBUG,"AuthenticationServer: scheme: "+profile.authentication_scheme);
			log(LoggerLevel.DEBUG,"AuthenticationServer: realm: "+profile.authentication_realm);
		}
		else as=null;
		...
		/*if (server_profile.memory_log) {
			...
			LoggerWriter memory_log=new LoggerWriter(filename,1);
	...
	/** When a new message is received.
	  * If the received message is a request, it checks for loops. */
	private void processReceivedMessage(SipProvider provider, SipMessage msg) {
		log(LoggerLevel.DEBUG,"message received");
		if (msg.isRequest()) {
			// it is a request
			log(LoggerLevel.DEBUG,"message is a request");
			...
			if (isResponsibleFor(target) && msg.hasRouteHeader()) {
				...
				if (!route_uri.hasLr()) {
					log(LoggerLevel.DEBUG,"probably the message was compliant to RFC2543 Strict Route rule: message is updated to RFC3261");
					...
					log(LoggerLevel.TRACE,"new recipient: "+target.toString());
			...
			log(LoggerLevel.TRACE,"is from local domain? "+((is_from_this_domain)?"yes":"no"));

			if (is_for_this_domain && (target.isSipURI() && !(new SipURI(target)).hasUserName())) {
				log(LoggerLevel.TRACE,"the recipient is this server");
				...
				log(LoggerLevel.TRACE,"the recipient is NOT this server");
				...
				if (is_for_this_domain) {
					log(LoggerLevel.TRACE,"the request is for a local user");
					// process the message
					processRequestToLocalUser(msg);
				}
				else {
					log(LoggerLevel.TRACE,"the request is for a remote UA");
					// process the message
			...
			if (msg.isResponse()) {
				log(LoggerLevel.TRACE,"message is a response");
				processResponse(msg);
			}
			else log(LoggerLevel.WARNING,"received message is not recognized as a request nor a response: discarded");
		}
	}
	...
	//{  printLog("sending the successfully processed message",LoggerWriter.LEVEL_MEDIUM);
	...
	protected SipMessage validateRequest(SipMessage msg) {
		log(LoggerLevel.TRACE,"inside validateRequest(msg)");
		...
		if (err_code>0) {
			log(LoggerLevel.INFO,"Message validation failed ("+err_code+" "+SipResponses.reasonOf(err_code)+"), message discarded");
