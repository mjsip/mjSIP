...
public class Proxy extends Registrar {
	...
	/** Creates a void Proxy */
	protected Proxy() {}


	/** Creates a new Proxy that acts also as location server for registered users. */
	...
	@Override
	protected void processRequestToLocalServer(SipMessage msg) {
	...
	protected SipMessage updateProxyingRequest(SipMessage msg) {
		log(LoggerLevel.TRACE,"inside updateProxyingRequest(msg)");
	...
	protected void processResponse(SipMessage resp) {
		...
		if (resp.hasViaHeader()) sip_provider.sendMessage(resp);
			...
			log(LoggerLevel.WARNING,"no VIA header found: message discarded");            
	}
	...
	protected SipMessage updateProxyingResponse(SipMessage resp) {
		log(LoggerLevel.DEBUG,"inside updateProxyingResponse(resp)");
		// clear transport information
		resp.clearTransport();
	...
	protected SipURI getAuthDomainBasedProxyingTarget(GenericURI request_uri) {
		log(LoggerLevel.TRACE,"inside getAuthDomainBasedProxyingTarget(uri)");
		// authenticated rules
		for (int i=0; i<server_profile.authenticated_domain_proxying_rules.length; i++) {
			...
			if (nexthop!=null) {
				log(LoggerLevel.DEBUG,"domain-based authenticated forwarding: "+rule.toString()+": YES");
				log(LoggerLevel.DEBUG,"target="+nexthop.toString());
				return nexthop;
			}
			else log(LoggerLevel.DEBUG,"domain-based authenticated forwarding: "+rule.toString()+": NO");
		}
		return null;
	}
	...
	protected SipURI getDomainBasedProxyingTarget(GenericURI request_uri) {
		log(LoggerLevel.TRACE,"inside getDomainBasedForwardingTarget(uri)");
		...
		for (int i=0; i<server_profile.domain_proxying_rules.length; i++) {
			...
			SipURI nexthop=rule.getNexthop(request_uri);
			if (nexthop!=null) {
				log(LoggerLevel.DEBUG,"domain-based forwarding: "+rule.toString()+": YES");
				log(LoggerLevel.DEBUG,"target="+nexthop.toString());
				return nexthop;
			}
			else log(LoggerLevel.DEBUG,"domain-based forwarding: "+rule.toString()+": NO");
		}
		return null;
	}
	...
	protected SipURI getAuthPrefixBasedProxyingTarget(GenericURI request_uri) {
		log(LoggerLevel.TRACE,"inside getAuthPrefixBasedProxyingTarget(uri)");
		...
		// else
		// authenticated rules
		log(LoggerLevel.TRACE,"authenticated prefix-based rules: "+server_profile.authenticated_phone_proxying_rules.length);
		for (int i=0; i<server_profile.authenticated_phone_proxying_rules.length; i++) {
			...
			SipURI nexthop=rule.getNexthop(request_uri);
			if (nexthop!=null) {
				log(LoggerLevel.DEBUG,"prefix-based authenticated forwarding: "+rule.toString()+": YES");
				log(LoggerLevel.DEBUG,"target="+nexthop.toString());
				return nexthop;
			}
			else log(LoggerLevel.DEBUG,"prefix-based authenticated forwarding: "+rule.toString()+": NO");
		}
		return null;
	}
	...
	protected SipURI getPrefixBasedProxyingTarget(GenericURI request_uri) {
		log(LoggerLevel.TRACE,"inside getPrefixBasedProxyingTarget(uri)");
		if (!request_uri.isSipURI())  return null;
		// else
		SipURI sip_uri=new SipURI(request_uri);
		String username=sip_uri.getUserName();
		...
		// else
		// non-authenticated rules
		log(LoggerLevel.TRACE,"prefix-based rules: "+server_profile.phone_proxying_rules.length);
		for (int i=0; i<server_profile.phone_proxying_rules.length; i++) {
			...
			SipURI nexthop=rule.getNexthop(request_uri);
			if (nexthop!=null) {
				log(LoggerLevel.DEBUG,"prefix-based forwarding: "+rule.toString()+": YES");
				log(LoggerLevel.DEBUG,"target="+nexthop.toString());
				return nexthop;
			}
			else log(LoggerLevel.DEBUG,"prefix-based forwarding: "+rule.toString()+": NO");
		}
		return null;
	}
	...
	public static void main(String[] args) {
		...
		boolean prompt_exit=flags.getBoolean("--prompt","prompts for exit");
