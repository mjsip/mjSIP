...
public class Registrar extends ServerEngine {
	...
	/** Creates a void Registrar. */
	protected Registrar() {}
	
	
	/** Creates a new Registrar. The Location Service is stored within the file <i>db_name</i> */
	...
	protected void processRequestToLocalUser(SipMessage msg) {
		...
		else log(LoggerLevel.INFO,"message discarded");
	}
	...
	protected void processResponse(SipMessage resp) {
		...
		// no actions..
		log(LoggerLevel.INFO,"message discarded");
	}
	...
	protected Vector getTargets(SipMessage msg) {
		log(LoggerLevel.TRACE,"inside getTargets(msg)");

		Vector targets=new Vector();
		
		if (location_service==null) {
			log(LoggerLevel.INFO,"Location service is not active");
			return targets;
		}           

		GenericURI request_uri=msg.getRequestLine().getAddress();
		if (!request_uri.isSipURI()) {
			log(LoggerLevel.INFO,"request-URI is not a SIP URI");
			return targets;
		}
		SipURI sip_uri=new SipURI(request_uri);
		String username=sip_uri.getUserName();
		if (username==null) {
			log(LoggerLevel.INFO,"no username found");
			return targets;
		}
		String user=username+"@"+sip_uri.getHost();
		log(LoggerLevel.DEBUG,"user: "+user); 
			  
		if (!location_service.hasUser(user)) {
			log(LoggerLevel.INFO,"user "+user+" not found");
			return targets;
		}
		...
		log(LoggerLevel.TRACE,"message targets: ");  
		for (int i=0; e.hasMoreElements(); i++) {
			...
			log(LoggerLevel.TRACE,"target"+i+" expired: contact URI removed");
			}
			// otherwise add the URI to the target list
			else {
				targets.addElement(contact);
				log(LoggerLevel.TRACE,"target"+i+"="+targets.elementAt(i));
			}
		}
		...
		if (request_uri.equals(GenericURI.SCHEME_SIPS)) {
			for (int i=0; i<targets.size(); i++) {
				...
				if (!uri.isSecure()) {
					log(LoggerLevel.INFO,uri.toString()+" has not SIPS scheme: skipped");
	...
	protected SipMessage updateRegistration(SipMessage msg) {
		...
		if (th==null)   {
			log(LoggerLevel.INFO,"ToHeader missed: message discarded");
			int result=400;
		...
		if (!location_service.hasUser(user)) {
			if (server_profile.register_new_users) {
				...
				log(LoggerLevel.INFO,"new user '"+user+"' added");
			} 
			else {
				log(LoggerLevel.INFO,"user '"+user+"' unknown: message discarded.");
				int result=404;
				return SipMessageFactory.createResponse(msg,result,null,null);  
			}
		}
		...
		if (!msg.hasContactHeader())   {
			//log(LoggerLevel.INFO,"ContactHeader missed: message discarded");
			//int result=484;
			//return SipMessageFactory.createResponse(msg,result,null,null,null);  
			log(LoggerLevel.DEBUG,"no contact found: fetching bindings..");
			int result=200;
			SipMessage resp=SipMessageFactory.createResponse(msg,result,null,null);  
		...
		if (ch_0.isStar()) {
			log(LoggerLevel.TRACE,"ContactHeader is star");
			...
			for (Enumeration e=location_service.getUserContactURIs(user); e.hasMoreElements();)  {
				...
				if (!location_service.isUserContactStatic(user,contact))  {
					...
					// update db
					location_service.removeUserContact(user,contact);
					log(LoggerLevel.TRACE,"contact removed: "+contact);
					if (exp_secs>0) {
						...
						//printLog("contact added: "+uri+"; expire: "+df.format(location_service.getUserContactExpire(user,url)),LoggerWriter.LEVEL_LOW);
						log(LoggerLevel.TRACE,"contact added: "+contact+"; expire: "+DateFormat.formatEEEddMMMyyyyhhmmss(location_service.getUserContactExpirationDate(user,contact)));
					}
			...
			for (int i=0; i<contacts.size(); i++)      {
				...
				if (exp_secs_i>0) {
					Date exp_date=new Date(System.currentTimeMillis()+((long)exp_secs)*1000);
					location_service.addUserContact(user,name_address,exp_date);
					log(LoggerLevel.INFO,"registration of user "+user+" updated");
				}           
