...
/** It manages invite dialogs.
  * It can works at either client or server side.
  ...
  * <p>
  * It supports the offer/answer model for the sip body, with the following rules:
...
public class InviteDialog extends Dialog {
	...
	/** Invite transaction server listener */
	InviteTransactionServerListener this_ts_listener=new InviteTransactionServerListener(){
		@Override
		public void onTransRequest(TransactionServer ts, SipMessage req) {
			processTransRequest(ts,req);
		}
		@Override
		public void onTransFailureAck(InviteTransactionServer ts, SipMessage ack) {
			processTransFailureAck(ts,ack);
		}
	};

	AckTransactionServerListener this_ack_ts_listener=new AckTransactionServerListener(){
		@Override
		public void onTransAckTimeout(AckTransactionServer ts) {
			processTransAckTimeout(ts);
		}
	};
	
	ReliableProvisionalResponderListener this_rel_resp_listener=new ReliableProvisionalResponderListener(){
		@Override
		public void onReliableProvisionalResponseConfirmation(ReliableProvisionalResponder rr, SipMessage resp, SipMessage prack) {
			processReliableProvisionalResponseConfirmation(rr,resp,prack);
		}
		@Override
		public void onReliableProvisionalResponseTimeout(ReliableProvisionalResponder rr, SipMessage resp) {
			processReliableProvisionalResponseTimeout(rr,resp);
		}	
	};
	...
	public InviteDialog(SipProvider sip_provider, SipMessage invite, InviteDialogListener listener) {
		...
		processReceivedMessage(sip_provider,invite);
	}
	...
	public void listen() {
		...
		invite_ts=new InviteTransactionServer(sip_provider,this_ts_listener);
		invite_ts.listen();
	}  
	...
	public void invite(NameAddress target, NameAddress from, NameAddress contact, String session_descriptor) {
		log(LoggerLevel.DEBUG,"inside invite(callee,caller,contact,sdp)");
		if (!statusIs(D_INIT)) return;
		// else
	...
	public void invite(SipMessage invite) {
		log(LoggerLevel.DEBUG,"inside invite(invite)");
		if (!statusIs(D_INIT)) return;
		// else
		...
		invite_tc=new InviteTransactionClient(sip_provider,invite,this_tc_listener);      
		invite_tc.request();
	}
	...
	public void reInvite(NameAddress contact, String session_descriptor) {
		log(LoggerLevel.DEBUG,"inside reInvite(contact,sdp)");
	...
	public void reInvite(SipMessage invite) {
		log(LoggerLevel.DEBUG,"inside reInvite(invite)");
		if (!statusIs(D_CALL)) return;
		// else
	...
	public void ring() {
		log(LoggerLevel.DEBUG,"inside ring()");
	...
	public void accept(NameAddress contact, String sdp) {
		log(LoggerLevel.DEBUG,"inside accept(contact,sdp)");
	...
	/** Refuses the incoming call.
	  * This method should be called when the InviteDialog is in D_INVITED or D_ReINVITED state. */
	public void refuse() {
		log(LoggerLevel.DEBUG,"inside refuse()");
		//refuse(480,null);
		//refuse(603,null);
		//refuse(403,null);
		refuse(486,null);
	...
	public void refuse(int code, String reason) {
		log(LoggerLevel.DEBUG,"inside refuse("+code+","+((reason!=null)?reason:SipResponses.reasonOf(code))+")");
	...
	public void redirect(int code, String reason, NameAddress contact) {
		log(LoggerLevel.DEBUG,"inside redirect("+code+","+reason+","+contact.toString()+")");
	...
	public void respond(int code, String reason, NameAddress contact, String content_type, byte[] body) {
		log(LoggerLevel.DEBUG,"inside respond("+code+","+reason+")");
		if (statusIs(D_INVITED) || statusIs(D_ReINVITED)) {
			...
			log(LoggerLevel.WARNING,"Dialog isn't in \"invited\" state: cannot respond ("+code+"/"+getStatus()+"/"+getDialogID()+")");
		}
	}
	...
	public void respond(SipMessage resp) {
		log(LoggerLevel.DEBUG,"inside respond(resp)");
		...
		if (method.equals(SipMethods.INVITE)) {
			if (!verifyStatus(statusIs(D_INVITED)||statusIs(D_ReINVITED))) {
				log(LoggerLevel.INFO,"respond(): InviteDialog not in (re)invited state: No response now");
				return;
			}     
			...
			if (code>=100 && code<200) {
				...
				if (code!=100 && ((invite_req.hasRequireHeader() && invite_req.getRequireHeader().hasOptionTag(SipStack.OTAG_100rel)) || (isExtensionRequired(SipStack.OTAG_100rel) && invite_req.hasSupportedHeader() && invite_req.getSupportedHeader().hasOptionTag(SipStack.OTAG_100rel)))) {
					if (reliable_responder==null) reliable_responder=new ReliableProvisionalResponder(invite_ts,this_rel_resp_listener);
					log(LoggerLevel.DEBUG,"respond(): reliable provisional response");
			...
			if (code>=200 && code<300) {
				...
				ack_ts=new AckTransactionServer(sip_provider,conn_id,invite_req,resp,this_ack_ts_listener);
				ack_ts.respond();
				//if (listener!=null)
	...
	public void confirm1xx(SipMessage prack) {
		if (prack.isPrack()) {
			log(LoggerLevel.INFO,"confirm1xx(): sending PRACK");
	...
	public void bye() {
		log(LoggerLevel.DEBUG,"inside bye()");
		if (statusIs(D_CALL)) {
	...
	public void bye(SipMessage bye) {
		log(LoggerLevel.DEBUG,"inside bye(bye)");
		if (statusIs(D_CALL)) {
			changeStatus(D_BYEING);
			//dialog_state.incLocalCSeq(); // done by SipMessageFactory.createRequest()
			TransactionClient tc=new TransactionClient(sip_provider,bye,this_tc_listener);
			tc.request();
	...
	public void cancel() {
		log(LoggerLevel.DEBUG,"inside cancel()");
	...
	public void cancel(SipMessage cancel) {
		log(LoggerLevel.DEBUG,"inside cancel(cancel)");
		if (statusIs(D_INVITING) || statusIs(D_ReINVITING)) {
			if (invite_tc.isProceeding())  {
	...
	public void info(String content_type, byte[] body) {
		log(LoggerLevel.DEBUG,"inside info(content_type,body)");
		if (statusIs(D_CALL)) {
	...
	public void info(SipMessage req) {
		log(LoggerLevel.DEBUG,"inside info(req)");
		if (statusIs(D_CALL)) {
			(new TransactionClient(sip_provider,req,this_tc_listener)).request();
		}
	}
	...
	public void update(NameAddress contact, String session_descriptor) {
		log(LoggerLevel.DEBUG,"inside update(contact,sdp)");
	...
	public void update(SipMessage req) {
		log(LoggerLevel.DEBUG,"inside update(update)");
		if (!(isEarly() || isConfirmed())) return;
		// else
	...
	public void acceptUpdate(String sdp) {
		log(LoggerLevel.DEBUG,"inside acceptUpdate(req)");
		if (update_ts!=null) {
	...
	public void refuseUpdate() {
		log(LoggerLevel.DEBUG,"inside refuseUpdate(req)");
		if (update_ts!=null) {
	...
	public void terminate() {
		log(LoggerLevel.INFO,"terminate()");
		if (!statusIs(D_CLOSE)) {
		...
		else log(LoggerLevel.DEBUG,"dialog already closed: nothing to do.");
	}


	// ************************* Callback methods *************************

	/** Called when a new message is received (out of any ongoing transaction) for the current InviteDialog.
	...
	@Override
	protected void processReceivedMessage(SipProvider sip_provider, SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside onReceivedMessage(sip_provider,message)");
		// if request
		if (msg.isRequest()) {
			// check CSeq
			if (!(msg.isAck() || msg.isCancel()) && msg.getCSeqHeader().getSequenceNumber()<=getRemoteCSeq()) {
				log(LoggerLevel.INFO,"Request message is too late (CSeq too small): Message discarded");
				return;
			}
			// else
			// SESSION TIMERS
			...
			if (msg.isInvite())       {
				...
				invite_ts=new InviteTransactionServer(sip_provider,msg,this_ts_listener);
				SipMessage resp=processInviteMessage(msg);
				if (resp!=null) respond(resp);
				else {
					cancel_ts=new CancelTransactionServer(sip_provider,invite_req,this_ts_listener);
					cancel_ts.listen();
					if (listener!=null) {
			...
			if (msg.isBye()) {
				...
				changeStatus(D_BYED);
				bye_ts=new TransactionServer(sip_provider,msg,this_ts_listener);
			...
			if (msg.isUpdate())       {
				...
				log(LoggerLevel.DEBUG,"onReceivedMessage(): is update");
				updateDialogInfo(false,msg);
	...
	protected void processTransProvisionalResponse(TransactionClient tc, SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside onTransProvisionalResponse(tc,mdg)");
	...
	protected void processTransFailureResponse(TransactionClient tc, SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside onTransFailureResponse("+tc.getTransactionId()+",msg)");
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
	...
	/** When an TransactionClientListener goes into the "Terminated" state, receiving a 2xx response.
	  ...
	  * If called for a BYE transaction, it moves to D_CLOSE state,
	  * removes the listener from SipProvider, and fires <i>onClose(this,msg)</i>. */
	protected void processTransSuccessResponse(TransactionClient tc, SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside onTransSuccessResponse(tc,msg)");
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
			if (!verifyStatus(statusIs(D_INVITING)||statusIs(D_ReINVITING))) return;
			StatusLine statusline=msg.getStatusLine();
			int code=statusline.getCode();
	...
	protected void processTransTimeout(TransactionClient tc) {
		log(LoggerLevel.DEBUG,"inside onTransTimeout(tc,msg)");
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
			if (!verifyStatus(statusIs(D_INVITING)||statusIs(D_ReINVITING))) return;
			changeStatus(D_CLOSE);
	...
	protected void processTransRequest(TransactionServer ts, SipMessage req) {
		log(LoggerLevel.DEBUG,"inside onTransRequest(ts,msg)");
		// INVITE
	...
	protected SipMessage processInviteMessage(SipMessage invite) {
	...
	/** When an InviteTransactionServer goes into the "Confirmed" state receiving an ACK for NON-2xx response.
	  * <p>
	  * It moves to D_CLOSE state and removes the listener from SipProvider. */
	protected void processTransFailureAck(InviteTransactionServer ts, SipMessage msg) {
		log(LoggerLevel.DEBUG,"inside onTransFailureAck(ts,msg)");
	...
	protected void processTransAckTimeout(AckTransactionServer ts) {
		log(LoggerLevel.DEBUG,"inside onAckSrvTimeout(ts)");
		if (!verifyStatus(statusIs(D_ACCEPTED)||statusIs(D_ReACCEPTED)||statusIs(D_REFUSED)||statusIs(D_ReREFUSED))) return;
		log(LoggerLevel.INFO,"No ACK received..");
		changeStatus(D_CLOSE);
		if (listener!=null) listener.onDlgClosed(this);
	}


	/** When a provisional response has been confirmed (PRACK). */
	protected void processReliableProvisionalResponseConfirmation(ReliableProvisionalResponder rr, SipMessage resp, SipMessage prack) {
	...
	/** When the retransmission timeout expired without receiving coinfirmation. */
	protected void processReliableProvisionalResponseTimeout(ReliableProvisionalResponder rr, SipMessage resp) {
