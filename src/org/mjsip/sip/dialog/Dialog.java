...
/** Class Dialog extends DialogInfo maintaining current dialog status ("early", "confirmed", or "terminated").
  */
public abstract class Dialog extends DialogInfo {
	...
	/** Called when a new message is received (out of any ongoing transaction) for the current dialog. */
	abstract protected void processReceivedMessage(SipProvider provider, SipMessage message);
	...
	protected void changeStatus(int newstatus) {
		status=newstatus;
		log(LoggerLevel.DEBUG,"changed dialog state: "+getStatus());
		...
		if (isEarly() || isConfirmed()) {
			if (dialog_id!=null && !sip_provider.getListeners().containsKey(dialog_id)) sip_provider.addSelectiveListener(dialog_id,this_sip_provider_listener);
		}
	}
	...
	public void updateDialogInfo(boolean is_client, SipMessage msg) {
		...
		if (isTerminated()) {
			log(LoggerLevel.WARNING,"trying to update a terminated dialog: do nothing.");
			return;
		}
		// else
		...
		if (secure_old!=secure) log(LoggerLevel.INFO,"secure dialog: on");
		...
		if (dialog_id==null || !dialog_id.equals(new_dialog_id)) {
			log(LoggerLevel.INFO,"new dialog-id: "+new_dialog_id);
			if (sip_provider!=null) sip_provider.addSelectiveListener(new_dialog_id,this_sip_provider_listener);
	...
	protected final void log(LoggerLevel level, Exception e) {
	...
	protected final boolean verifyThat(boolean expression, String str) {
		if (!expression) {
			if (str==null || str.length()==0) log(LoggerLevel.WARNING,"expression check failed. ");
			else log(LoggerLevel.WARNING,str);
		}
		return expression;
	}
