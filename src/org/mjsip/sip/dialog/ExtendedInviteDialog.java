...
public class ExtendedInviteDialog extends InviteDialog {
	...
	public void respond(SipMessage resp) {
		log(LoggerLevel.DEBUG,"inside x-respond(resp)");
		...
		if (method.equals(SipMethods.INVITE) || method.equals(SipMethods.CANCEL) || method.equals(SipMethods.BYE)) {
			...
			log(LoggerLevel.DEBUG,"transaction-id="+transaction_id);
			if (transactions.containsKey(transaction_id)) {
				...
				log(LoggerLevel.DEBUG,"transaction server not found; message discarded");
			}
		}
	} 
	...
	public void acceptRefer(SipMessage req) {
		log(LoggerLevel.DEBUG,"inside acceptRefer(refer)");
	...
	public void refuseRefer(SipMessage req) {
		log(LoggerLevel.DEBUG,"inside refuseRefer(refer)");
	...
	@Override
	protected void processReceivedMessage(SipProvider provider, SipMessage msg) {
		log(LoggerLevel.TRACE,"onReceivedMessage(): "+msg.getFirstLine().substring(0,msg.toString().indexOf('\r')));
		if (msg.isResponse()) {
			super.processReceivedMessage(provider,msg);
		}
		else
		if (msg.isInvite() || msg.isAck() || msg.isCancel() || msg.isBye() || msg.isInfo() || msg.isPrack()) {
			...
			TransactionServer t=new TransactionServer(sip_provider,msg,this_ts_listener);
			transactions.put(t.getTransactionId(),t);
			...
			if (msg.isNotify()) {
				...
				log(LoggerLevel.DEBUG,"Received alternative request "+msg.getRequestLine().getMethod());
	...
	protected void processTransFailureResponse(TransactionClient tc, SipMessage msg) {
		...
		if ((code==401 && attempts<MAX_ATTEMPTS && msg.hasWwwAuthenticateHeader() && msg.getWwwAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm))
		 || (code==407 && attempts<MAX_ATTEMPTS && msg.hasProxyAuthenticateHeader() && msg.getProxyAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm)))   {
			...
			if (req.isInvite()) tc=new InviteTransactionClient(sip_provider,req,this_tc_listener);
			else tc=new TransactionClient(sip_provider,req,this_tc_listener);
	...
	protected void processTransSuccessResponse(TransactionClient t, SipMessage msg) {
		...
		if (method.equals(SipMethods.INVITE) || method.equals(SipMethods.CANCEL) || method.equals(SipMethods.BYE)) {
			super.processTransSuccessResponse(t,msg);
		}
		else
