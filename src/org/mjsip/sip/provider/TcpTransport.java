...
public class TcpTransport extends SipTransportCO {
	 ...
	 * @param local_port TCP port
	 * @param nmax_connections maximum number of concurrent TCP connections
	 * @throws IOException */
	...
	/** Creates a new TcpTransport.
	 * @param local_port TCP port
	 * @param host_ipaddr IP address to bound to
	 * @param nmax_connections maximum number of concurrent TCP connections
	 * @throws IOException */
	...
	private void processIncomingConnection(TcpServer tcp_server, TcpSocket socket) {
		log(LoggerLevel.DEBUG,"incoming connection from "+socket.getAddress()+":"+socket.getPort());
		if (tcp_server==this.tcp_server) {
			SipTransportConnection conn=new TcpTransportConnection(socket,this_conn_listener);
			log(LoggerLevel.DEBUG,"tcp connection "+conn+" opened");
			addConnection(conn);
	...
	private void processServerTerminated(TcpServer tcp_server, Exception error)  {
		log(LoggerLevel.DEBUG,"tcp server "+tcp_server+" terminated");
	}
