...
public abstract class SipTransportCO implements SipTransport/*, SipTransportConnectionListener*/ {
	 ...
	 * @param local_port the transport port
	 * @param nmax_connections maximum number of concurrent connections
	 * @throws IOException */
	public SipTransportCO(int local_port, int nmax_connections) throws IOException {
	...
	public ConnectionId sendMessage(SipMessage msg, IpAddress dest_ipaddr, int dest_port, int ttl) throws IOException {
		...
		synchronized (connections) {
			...
			try {
				log(LoggerLevel.DEBUG,"already active connection found for connection-id "+connection_id);
				SipTransportConnection conn=(SipTransportConnection)connections.get(connection_id);
				log(LoggerLevel.DEBUG,"sending data through already active connection "+conn);
				...
				log(LoggerLevel.DEBUG,"error using previous connection with connection-id "+connection_id);
				removeConnection(connection_id);
			}
			// no active connection
			log(LoggerLevel.DEBUG,"no active connection for "+connection_id);
			if (!manual) {
				// AUTOMATIC CONN MODE
				log(LoggerLevel.DEBUG,"open "+getProtocol()+" connection to "+dest_ipaddr+":"+dest_port);
				try {
					SipTransportConnection conn=addConnection(dest_ipaddr,dest_port);
					if (conn!=null) {
						log(LoggerLevel.DEBUG,"sending data through connection "+conn);
						//conn.sendMessage(msg);
						sendMessage(conn,msg);
						return new ConnectionId(conn);
					}
					else {
						log(LoggerLevel.DEBUG,"null connection: message has not been sent");
						return null;
					}
				}
				catch (Exception e) {
				...
				// MANUAL CONN MODE
				log(LoggerLevel.DEBUG,"only manual connections: message has not been sent");
				return null;
			}
		}
		// END SYNCHRONIZATION      
	...
	public SipTransportConnection sendMessageCO(SipMessage msg) throws IOException  {
		...
		synchronized (connections) {
			...
			// else
			log(LoggerLevel.DEBUG,"active connection found matching "+connection_id);
			SipTransportConnection conn=(SipTransportConnection)connections.get(connection_id);
			//conn.sendMessage(msg);
			sendMessage(conn,msg);
			return conn;
	...
	public void halt() {
		...
		if (connections!=null) {
			log(LoggerLevel.TRACE,"connections are going down");
	...
	protected void processConnectionTerminated(SipTransportConnection conn, Exception error) {
		...
		removeConnection(connection_id);
		log(LoggerLevel.INFO,"connection "+conn+" terminated");
		if (error!=null) log(LoggerLevel.INFO,error);
	...
	public SipTransportConnection addConnection(IpAddress remote_ipaddr, int remote_port) throws IOException {
		...
		if (conn!=null)  {
			log(LoggerLevel.INFO,"connection "+conn+" opened");
			addConnection(conn);
		}
		else log(LoggerLevel.INFO,"no connection has been opened");
		return conn;
	}
	...
	public void addConnection(SipTransportConnection conn) {
		...
		synchronized (connections) {
			...
			if (connections.containsKey(connection_id)) {
				...
				log(LoggerLevel.INFO,"trying to add the already established connection "+connection_id);
				log(LoggerLevel.INFO,"connection "+connection_id+" will be replaced");
			...
			if (connections.size()>=nmax_connections) {
				...
				log(LoggerLevel.INFO,"reached the maximum number of connection: removing the older unused connection");
			...
			// DEBUG log:
			log(LoggerLevel.TRACE,"active connenctions:");
			for (Enumeration e=connections.keys(); e.hasMoreElements(); ) {
				ConnectionId id=(ConnectionId)e.nextElement();
				log(LoggerLevel.TRACE,"connection-id="+id+": "+((SipTransportConnection)connections.get(id)).toString());
			}
		}
		// END SYNCHRONIZATION      
	}
	...
	public void removeConnection(ConnectionId connection_id) {
		...
		synchronized (connections) {
			...
			if (connections.containsKey(connection_id)) {
				...
				for (Enumeration e=connections.elements(); e.hasMoreElements(); ) {
					SipTransportConnection co=(SipTransportConnection)e.nextElement();
					log(LoggerLevel.TRACE,"conn "+co.toString());
				}
			}
		}
		// END SYNCHRONIZATION
	...
	void log(LoggerLevel level, Exception e) {
