...
public class RegistrationClient {
	...
	protected void register(int expire_time, String content_type, byte[] body) {
		log(LoggerLevel.INFO,"register with "+registrar_uri+" for "+expire_time+" secs");
		attempts=0;
		...
		if (body!=null) {
			log(LoggerLevel.INFO,"register body type: "+content_type+"; length: "+body.length+" bytes");
			req.setBody(content_type,body);
		}
		if (expire_time>0) log(LoggerLevel.INFO,"registering contact "+contact_naddr+" (it expires in "+expire_time+" secs)");
		else log(LoggerLevel.INFO,"unregistering contact "+contact_naddr);
	...
	public void unregisterall() {
		...
		log(LoggerLevel.INFO,"unregistering all contacts");
		TransactionClient t=new TransactionClient(sip_provider,req,this_tc_listener); 
		t.request(); 
	}
	...
	/** When client sends back a provisional response. */
	private void processTransProvisionalResponse(TransactionClient transaction, SipMessage resp) {
		// do nothing..
	}

	/** When client sends back a success response. */
	private void processTransSuccessResponse(TransactionClient transaction, SipMessage resp) {
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			...
			log(LoggerLevel.INFO,"Registration success: expires: "+expires+"s: "+result);
			if (loop) {
				attempt_to=null;
				(registration_to=new Timer((long)renew_time*1000,this_timer_listener)).start();
				log(LoggerLevel.TRACE,"next registration after "+renew_time+" secs");
			}
			if (listener!=null) listener.onRegistrationSuccess(this,to_naddr,contact_naddr,expires,result);
		}
	}

	/** When client sends back a failure response. */
	private void processTransFailureResponse(TransactionClient transaction, SipMessage resp) {
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			StatusLine status=resp.getStatusLine();
			...
			if (code==401 && attempts<SipStack.regc_auth_attempts && resp.hasWwwAuthenticateHeader() && resp.getWwwAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm)) {
				...
				//log(LoggerLevel.DEBUG,"qop-options: "+qop_options);
			...
			if (code==407 && attempts<SipStack.regc_auth_attempts && resp.hasProxyAuthenticateHeader() && resp.getProxyAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm)) {
				...
				if (loop) {
					registration_to=null;
					(attempt_to=new Timer(SipStack.regc_max_attempt_timeout,this_timer_listener)).start();
					log(LoggerLevel.TRACE,"next attempt after "+(SipStack.regc_max_attempt_timeout/1000)+" secs");
				}
	...
	/** When client expires timeout. */
	private void processTransTimeout(TransactionClient transaction) {
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			log(LoggerLevel.INFO,"Registration failure: No response from server");
			if (loop) {
				registration_to=null;
				...
				(attempt_to=new Timer(inter_time_msecs,this_timer_listener)).start();
				log(LoggerLevel.TRACE,"next attempt after "+(inter_time_msecs/1000)+" secs");
			}
