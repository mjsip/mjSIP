...
/** It implements a SIP call.
  * It extends basic {@link Call} in order to support:
  * <ul>
  * <li>call transfer (REFER/NOTIFY methods), </li>
  * <li>UAS authentication and proxy authentication. </li>
  * </ul>
  */
public class ExtendedCall extends Call {
	...
	/** Creates a new call at the caller's side.
	...
	/** Creates a new call at the callee's side, based on an already received INVITE request.
	  * @param sip_provider the SIP provider
	...
	public void attendedTransfer(NameAddress transfer_to, Call replaced_call) {
		((ExtendedInviteDialog)dialog).refer(transfer_to,user_naddr,replaced_call.dialog);
	}
	...
	private void processDlgRefer(org.mjsip.sip.dialog.InviteDialog d, NameAddress refer_to, NameAddress referred_by, SipMessage msg) {
		...
		log(LoggerLevel.TRACE,"onDlgRefer("+refer_to.toString()+")");       
		refer=msg;
	...
	private void processDlgReferResponse(org.mjsip.sip.dialog.InviteDialog d, int code, String reason, SipMessage msg) {
		if (d!=dialog) {  log(LoggerLevel.INFO,"NOT the current dialog");  return;  }
		log(LoggerLevel.TRACE,"onDlgReferResponse("+code+" "+reason+")");       
