...
public class ReliableProvisionalResponder {
	...
	public ReliableProvisionalResponder(InviteTransactionServer invite_ts, ReliableProvisionalResponderListener listener) {
		...
		log(LoggerLevel.INFO,"new ReliableProvisionalResponder has been created");
	}    
	...
	public synchronized void processPrack(SipMessage prack) {
		log(LoggerLevel.DEBUG,"processPrack()");
		if (responses.size()>0) {
			...
			else log(LoggerLevel.WARNING,prack.getRequestLine().getMethod()+" confirmation received for past response?");
		}
		else log(LoggerLevel.WARNING,prack.getRequestLine().getMethod()+" no provisional response waiting for confirmation has been found");
	}
