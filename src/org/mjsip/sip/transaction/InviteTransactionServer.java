	...
	public void respondWith(SipMessage resp) {
		...
		if (code>=300 && code<700 && (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING))) {
			...
			if (connection_id==null) {
				...
				log(LoggerLevel.TRACE,"No retransmissions for reliable transport ("+connection_id+")");
				//onTimeout(end_to);
			}
			end_to=new Timer(end_to.getTime(),this_timer_listener);
			end_to.start();
		}
	}
	...
	private void processTimeout(Timer to) {
		try {
			...
			if (to.equals(end_to) && statusIs(STATE_COMPLETED)) {
				log(LoggerLevel.INFO,"End timeout expired");
				doTerminate();
				invite_ts_listener=null;
			}  
