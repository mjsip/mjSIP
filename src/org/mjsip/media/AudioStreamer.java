...
/** Full-duplex audio streamer based on javax.sound.
  */
public class AudioStreamer implements MediaStreamer {
	...
	/** RTP stream sender listener */
	RtpStreamSenderListener this_rtp_stream_sender_listener=new RtpStreamSenderListener(){
		@Override
		public void onRtpStreamSenderTerminated(RtpStreamSender rs, Exception error) {
			processRtpStreamSenderTerminated(rs,error);
		}};
	
	/** RTP stream receiver listener */
		RtpStreamReceiverListener this_rtp_stream_receiver_listener=new RtpStreamReceiverListener(){
		@Override
		public void onRemoteSoAddressChanged(RtpStreamReceiver rr, SocketAddress remote_soaddr) {
			processRemoteSoAddressChanged(rr,remote_soaddr);
		}
		@Override
		public void onRtpStreamReceiverTerminated(RtpStreamReceiver rr, Exception error) {
			processRtpStreamReceiverTerminated(rr,error);
		}};
	...
	public AudioStreamer(RtpStreamSender rtp_sender, RtpStreamReceiver rtp_receiver, boolean symmetric_rtp) {
		...
		log(LoggerLevel.DEBUG,"codec: [unknown]");
	}
	...
	private void init(int local_port, String remote_addr, int remote_port, FlowSpec.Direction direction, String audiofile_in, String audiofile_out, String codec_name, int payload_type, int sample_rate, int channels, int packet_size, boolean explicit_conversion, Codec additional_encoding, boolean do_sync, int random_early_drop, boolean symmetric_rtp, boolean rtcp) {
		...
		log(LoggerLevel.DEBUG,"codec: "+codec_name);     
		if (!codec_name.equals(codec.toString())) log(LoggerLevel.DEBUG,"codec mapped to: "+codec);
		...
		log(LoggerLevel.DEBUG,"packet size: "+packet_size+ "B");
		log(LoggerLevel.DEBUG,"packet time: "+packet_time+ "ms");
		log(LoggerLevel.DEBUG,"packet rate: "+(1000/packet_time)+ "pkt/s");
	
		// 4) find the proper supported AudioFormat
		log(LoggerLevel.DEBUG,"base audio format: "+SimpleAudioSystem.getBaseAudioFormat(sample_rate,channels).toString());
		...
		log(LoggerLevel.INFO,"Supported codecs: "+supported_list.toString());
		...
		for (int i=0; i<supported_encodings.length ; i++) {
			//printLog("supported_encoding["+i+"]: "+supported_encodings[i],LoggerWriter.LEVEL_HIGH);
			if (supported_encodings[i].toString().equalsIgnoreCase(codec_str))  {
				encoding=supported_encodings[i];
				//printLog("supported_encoding["+i+"]: OK",LoggerWriter.LEVEL_HIGH);
				break;
			}
		}
		if (encoding!=null) {
			...
			log(LoggerLevel.INFO,"target audio format: "+audio_format);
			//log(LoggerLevel.DEBUG,"frame_size: "+audio_format.getFrameSize());
			//log(LoggerLevel.DEBUG,"frame_rate: "+audio_format.getFrameRate());
			//log(LoggerLevel.DEBUG,"big_endian: "+audio_format.isBigEndian());
		}
		else log(LoggerLevel.WARNING,"codec '"+codec+"' not natively supported");
		...
		if (codec.equals(CodecType.AMR_NB) || codec.equals(CodecType.AMR_0475) || codec.equals(CodecType.AMR_0515) || codec.equals(CodecType.AMR_0590) || codec.equals(CodecType.AMR_0670) || codec.equals(CodecType.AMR_0740) || codec.equals(CodecType.AMR_0795) || codec.equals(CodecType.AMR_1020) || codec.equals(CodecType.AMR_1220)) {
			...
			if (VERBOSE_DEBUG) println("X-DEBUG: "+SystemUtils.getSimpleClassName(AudioStreamer.class.getName())+": RTP format: "+codec+" in "+((RTP_BANDWIDTH_EFFICIENT_MODE)? "Bandwidth-Efficinet" : "Octect-Alignied")+" Mode");
		}
		...
		try {
			...
			if ((dir==FlowSpec.SEND_ONLY || dir==FlowSpec.FULL_DUPLEX)) {
				log(LoggerLevel.DEBUG,"new audio sender to "+remote_addr+":"+remote_port);
				if (audiofile_in!=null && audiofile_in.equals(AudioStreamer.TONE)) {
					...
					// sender
					rtp_sender=new RtpStreamSender(tone,true,payload_type,sample_rate,channels,packet_time,packet_size,additional_encoder,udp_socket,remote_addr,remote_port,this_rtp_stream_sender_listener);
				}
				else
				if (audiofile_in!=null) {
					AudioInputStream audio_input_stream=AudioFile.getAudioFileInputStream(audiofile_in,audio_format);
					rtp_sender=new RtpStreamSender(audio_input_stream,true,payload_type,sample_rate,channels,packet_time,packet_size,additional_encoder,udp_socket,remote_addr,remote_port,this_rtp_stream_sender_listener);
				}
				else {
					// javax sound
					...
					// sender
					rtp_sender=new RtpStreamSender(audio_input_stream,do_sync,payload_type,sample_rate,channels,packet_time,packet_size,additional_encoder,udp_socket,remote_addr,remote_port,this_rtp_stream_sender_listener);
			...
			if (dir==FlowSpec.RECV_ONLY || dir==FlowSpec.FULL_DUPLEX) {
				log(LoggerLevel.DEBUG,"new audio receiver on "+local_port);
				if (audiofile_out!=null) {
					...
					// receiver
					rtp_receiver=new RtpStreamReceiver(audio_output_stream,additional_decoder,udp_socket,this_rtp_stream_receiver_listener);
		...
		catch (Exception e) {  log(LoggerLevel.INFO,e);  }
	...
	public boolean start() {
		log(LoggerLevel.INFO,"starting java audio");
		if (rtp_sender!=null) {
			log(LoggerLevel.DEBUG,"start sending");
			if (audio_input) SimpleAudioSystem.startAudioInputLine();
			rtp_sender.start();
		}
		if (rtp_receiver!=null) {
			log(LoggerLevel.DEBUG,"start receiving");
	...
	/** Stops media streams. */
	public boolean halt() {
		log(LoggerLevel.INFO,"stopping java audio");    
		if (rtp_sender!=null) {
			rtp_sender.halt();
			rtp_sender=null;
			log(LoggerLevel.DEBUG,"sender halted");
		}      
		...
		if (rtp_receiver!=null) {
			rtp_receiver.halt();
			rtp_receiver=null;
			log(LoggerLevel.DEBUG,"receiver halted");
		}      
	...
	private void processRemoteSoAddressChanged(RtpStreamReceiver rr, SocketAddress remote_soaddr) {
		try {
			if (symmetric_rtp && rtp_sender!=null) rtp_sender.setDestSoAddress(remote_soaddr);
		}
		catch (Exception e) {  log(LoggerLevel.INFO,e);  }
	}
	...
	private void processRtpStreamReceiverTerminated(RtpStreamReceiver rr, Exception error) {
		if (error!=null) log(LoggerLevel.INFO,error.getMessage());
	}


	/** When the stream sender terminated. */
	private void processRtpStreamSenderTerminated(RtpStreamSender rs, Exception error) {
		if (error!=null) log(LoggerLevel.INFO,error.getMessage());
	}
	...
	protected void log(LoggerLevel level, String str) {
		...
		if (level.getValue()>=LoggerLevel.INFO.getValue()) System.out.println(SystemUtils.getSimpleClassName(getClass().getName())+": "+str);
	}
