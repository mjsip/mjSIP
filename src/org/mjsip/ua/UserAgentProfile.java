...
public class UserAgentProfile extends Configure {
	...
	/** Rate of keep-alive tokens (datagrams) sent toward the outbound proxy
	  ...
	  * Its value specifies the delta-time (in milliseconds) between two
  ...
  public static String res_path="resources";
	...
	/** Creates a void UA profile. */
	public UserAgentProfile() {
		init();
	}

	/** Creates a new UA profile.
	...
	public void setAOR(NameAddress naddr) {
		...
		display_name=naddr.getDisplayName();
		user=uri.getUserName();
		proxy=(uri.hasPort())? uri.getHost()+":"+uri.getPort() : uri.getHost();
