...
public class MediaAgent {
	...
	public boolean startMediaSession(FlowSpec flow_spec) {
		...
		if (ua_profile.loopback) media_streamer=new LoopbackMediaStreamer(flow_spec);
		else
		...
		if (media_streamer!=null) {
			...
			log(LoggerLevel.WARNING,"No "+media+" streamer has been found: "+media+" not started");
			return false;
		}
	}
	...
	public void stopMediaSession(String media) {
		...
		if (media_streamers.containsKey(media)) {
			...
			log(LoggerLevel.WARNING,"No running "+media+" streamer has been found.");
		}
	}
	...
	private MediaStreamer newAudioStreamer(FlowSpec audio_flow) {
		...
		if (ua_profile.use_jmf_audio) {
			...
			try {
				...
				if (ua_profile.recv_file!=null) log(LoggerLevel.WARNING,"File destination is not supported with JMF audio");
				Class media_streamer_class=Class.forName("org.mjsip.ext.media.jmf.JmfMediaStreamer");
				...
				log(LoggerLevel.WARNING,e);
				log(LoggerLevel.SEVERE,"Error trying to create the JmfMediaApp");
			}
		}
		// else
		if (audio_streamer==null) {
			...
			if (ua_profile.javax_sound_streamer==null) {
				...
				try {
					...
					log(LoggerLevel.WARNING,e);
					log(LoggerLevel.SEVERE,"Error trying to create audio streamer '"+ua_profile.javax_sound_streamer+"'");
				}
			}
		}
	...
	private MediaStreamer newVideoStreamer(FlowSpec video_flow) {
		...
		if (ua_profile.use_jmf_video) {
			...
			try {
				...
				if (ua_profile.recv_video_file!=null) log(LoggerLevel.WARNING,"File destination is not supported with JMF video");
				Class media_streamer_class=Class.forName("org.mjsip.ext.media.jmf.JmfMediaApp");
				Class[] param_types={ FlowSpec.class, String.class };
	...
	private MediaStreamer newPttStreamer(FlowSpec flow_spec) {
		...
		try {
			Class media_streamer_class=Class.forName("org.mjsip.ext.media.push2talk.Push2TalkApp");
			Class[] param_types={ FlowSpec.class };
			...
			log(LoggerLevel.WARNING,e);
			log(LoggerLevel.SEVERE,"Error trying to create the Push2TalkApp");
		}
		return ptt_streamer;
	}
